...
pub struct SourceMeter {
    source_index: u32,
    /// 持有 record stream 引用以保活（Drop 时自动释放）。
    #[allow(dead_code)]
...
impl SourceMeter {
    ...
    pub fn create(ctx: &mut Context, source_index: u32) -> Result<Arc<Self>, String> {
        let spec = Spec {
...

        {
            ...
            s.connect_record(Some(&source_index.to_string()), Some(&attr), flags).map_err(|e| {
                format!("connect monitor stream failed for source {source_index}: {e}")
            })?;
        }
